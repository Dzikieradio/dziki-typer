const U='https://kfysmqwhpzemoknqakzn.supabase.co',K='sb_publishable_UNdKtQ2mCMaaWcMK0FvVIA_r8cNIUl-';const db=supabase.createClient(U,K);const $=id=>document.getElementById(id);let user=null,picks={},filter='all',profile=null,isAdmin=false,results={},goalEvents={},adminMatchFilter='all',adminUsersCache=[],presenceChannel=null,onlineUsers={};const names={okregowa:'Liga Okręgowa Skoczów–Żywiec',a:'A Klasa Żywiec',b:'B Klasa Żywiec',cup:'🏆 Puchar Polski • Podokręg Żywiec'};async function login(){message.textContent='Logowanie...';const{data,error}=await db.auth.signInWithPassword({email:email.value.trim(),password:password.value});if(error){message.textContent='❌ '+error.message;return}await enter(data.user)}async function register(){
...
function recentlyFinished(r){
 if(!r||r.status!=='finished'||!r.updated_at)return false;
 const finishedAt=new Date(r.updated_at).getTime();
 return Number.isFinite(finishedAt)&&Date.now()-finishedAt<24*60*60*1000;
}
function renderLive(){const active=MATCHES.filter(m=>!m._archived&&results[m.id]?.status==='live');const finished=MATCHES.filter(m=>!m._archived&&recentlyFinished(results[m.id])).sort((a,b)=>new Date(results[b.id].updated_at)-new Date(results[a.id].updated_at));let html='';if(active.length){html+=active.map(m=>{const r=results[m.id];const p=picks[m.id];return `<div class="livecard"><div class="liveMeta"><span class="livePill">${liveMinute(r)===45?'⏸ PRZERWA':`● LIVE${liveMinute(r)?` • ${liveMinute(r)}'`:''}`}</span><span>${m.label}</span></div><div class="liveTeams"><span>${m.home}</span><b>${r.home_score??0} : ${r.away_score??0}</b><span>${m.away}</span></div>${goalsHtml(m.id)}${p?`<div class="yourPick">Twój typ: <b>${p.home_score} : ${p.away_score}</b></div>`:''}</div>`}).join('')}else html+='<div class="emptyLive">Teraz nie trwa żaden mecz.</div>';if(finished.length){html+='<h3 class="recentTitle">Ostatnio zakończone (24 godz.)</h3>'+finished.slice(0,6).map(m=>{const r=results[m.id];return `<div class="finishedRow"><span>${m.home} – ${m.away}</span><b>${r.home_score} : ${r.away_score}</b></div>`}).join('')}$('liveMatches').innerHTML=html}function render(){let html='';['okregowa','a','b','cup'].forEach(l=>{if(filter!=='all'&&filter!==l)return;const ms=MATCHES.filter(m=>m.league===l);if(ms.length)html+=`<h3 class="league league-${l}">${names[l]}</h3>`+ms.map(card).join('')});if(filter==='all'){const cms=MATCHES.filter(m=>m._custom&&!m._archived);const groups={};cms.forEach(m=>(groups[m.competition]??=[]).push(m));Object.entries(groups).forEach(([title,ms])=>html+=`<h3 class="league">🏆 ${esc(title)}</h3>`+ms.map(card).join(''));}$('matches').innerHTML=html}
