...
function resultBadge(m){const r=results[m.id];if(!r||r.status==='scheduled')return '';if(r.status==='live'){const lm=liveMinute(r);return `<div class="resultline live"><span>🔴 NA ŻYWO${lm?` • ${lm}'`:''}</span><b>${r.home_score??0} : ${r.away_score??0}</b></div>`;}if(r.status==='finished')return `<div class="resultline finished"><span>KONIEC</span><b>${r.home_score} : ${r.away_score}</b></div>`;return ''}
