...
function editMyProfile(){
  ...
  box.innerHTML=`<h3>✏️ Edytuj profil</h3>
    <p class="muted">Twój nick:</p>
    <input id="profileNickname" maxlength="24" value="${esc(profile?.nickname||'')}" placeholder="np. Prezes">
...
async function saveProfileLook(){
  const nickname=$('profileNickname').value.trim(),avatar=$('selectedAvatar').value||'👤',favorite_club=$('favoriteClub').value||null;
  if(nickname.length<2){alert('Nick musi mieć co najmniej 2 znaki.');return}
  const{error}=await db.from('profiles').upsert({id:user.id,nickname,avatar,favorite_club},{onConflict:'id'});
