...
async function loadAdmin(){
  if(!isAdmin)return;
  const{data,error}=await db.from('match_results').select('match_id,home_score,away_score');
  if(error){$('adminMatches').innerHTML='<div class="warn">Nie udało się pobrać wyników.</div>';return}
  results={};(data||[]).forEach(r=>results[r.match_id]=r);
  $('adminMatches').innerHTML=MATCHES.map(m=>{
    const r=results[m.id]||{};
    return `<div class="adminmatch"><div class="adminteams">${m.home}<br>— ${m.away}</div><div class="date">${m.label}</div><div class="adminscore"><input id="rh-${m.id}" type="number" min="0" inputmode="numeric" value="${r.home_score??''}"><span>:</span><input id="ra-${m.id}" type="number" min="0" inputmode="numeric" value="${r.away_score??''}"><button onclick="saveResult('${m.id}')">Zapisz wynik</button></div></div>`;
  }).join('');
}
async function saveResult(id){
  if(!isAdmin)return;
  const h=Number($('rh-'+id).value),a=Number($('ra-'+id).value);
  if(!Number.isInteger(h)||!Number.isInteger(a)||h<0||a<0){alert('Wpisz oba wyniki jako liczby 0 lub większe.');return}
  const{error}=await db.from('match_results').upsert({match_id:id,home_score:h,away_score:a,updated_at:new Date().toISOString()},{onConflict:'match_id'});
  if(error){alert('Nie udało się zapisać wyniku: '+error.message);return}
  $('adminMsg').textContent='✓ Wynik zapisany. Ranking został przeliczony.';
  await loadAdmin();
}
...
async function loadRanking(){const{data,error}=await db.rpc('full_ranking');if(error){$('ranking').textContent='Nie udało się pobrać rankingu.';return}const rows=data||[];$('ranking').innerHTML='<div class="rankrow rankhead"><span>#</span><span>Gracz</span><span class="rankcount">Pkt</span></div>'+rows.map((r,i)=>`<div class="rankrow"><span class="ranknum">${i+1}</span><span>${r.nickname}<small style="display:block;color:#9fc7bb">🎯 ${r.exact_scores} • typy ${r.picks_count}</small></span><span class="rankcount"><b>${r.points}</b></span></div>`).join('')}async function showTab(t){
  const m=t==='matches',r=t==='ranking',a=t==='admin';
